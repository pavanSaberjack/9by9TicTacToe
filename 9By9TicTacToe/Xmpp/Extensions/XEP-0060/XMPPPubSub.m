...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
    ...
    {
        // Example get previous items for node
		//
		// <iq type='result' from='pubsub.shakespeare.lit' to='hamlet@denmark.lit/elsinore' id='items1'/>
		//      <pubsub xmlns='http://jabber.org/protocol/pubsub'>
        //          <items node='princely_musings'>
        //              <item> adfds fdsfsd f </item>
        //          </items>
        // </iq>
        //
        // Need to define error format

		
		if ([[iq type] isEqualToString:@"result"])
			[multicastDelegate xmppPubSub:self didReceivePreviousItemsForNode:node withResult:iq];
		else
			[multicastDelegate xmppPubSub:self didNotReceivePreviousItemsForNode:node withResult:iq];
...
- (NSString *)retrivePreviousItemsForNode:(NSString *)node
{
    NSString *uuid = [xmppStream generateUUID];
    ...
    NSXMLElement *item = [NSXMLElement elementWithName:@"items"];

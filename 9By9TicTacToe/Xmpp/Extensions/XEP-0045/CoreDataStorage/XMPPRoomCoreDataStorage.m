...
- (void)didInsertMessage:(XMPPRoomMessageCoreDataStorageObject *)message
{
    ...
    // PI: send notification
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:@"XMPPManagerRoomDidInsertMessageNotification" object:nil];
    });
}
               ...
               stream:(XMPPStream *)xmppStream
{
    ...
    NSDictionary *dictionary = nil;//[ZingMessageFactory getDictionaryForString:[body stringValue]];
    
    /*
    if (dictionary != nil)
    {
        NSString *senderValue = dictionary[@"sender"];
        BOOL isMessageOutGoing = FALSE;
        if ([senderValue isEqualToString:[[SessionManager sharedInstance] currentSession].jabberId])
        {
            isMessageOutGoing = YES;
        }
        
        roomMessage.senderJid = senderValue;
        roomMessage.customTimeStamp = dictionary[@"timestamp"];
        
        NSString *type = dictionary[@"type"]; // [[zingelement attributeForName:@"type"] stringValue];
        if ([type isEqualToString:@"comment"])
        {
            roomMessage.type = isMessageOutGoing == TRUE?   @(MessageTypeOutgoingComment) : @(MessageTypeIncomingComment);
            roomMessage.commentStr = dictionary[@"comment"]; // [[zingelement elementForName:@"comment"] stringValue];
            roomMessage.commentDetailStr = dictionary[@"subcomment"]; //[[zingelement elementForName:@"subcomment"] stringValue];
        }
        else if ([type isEqualToString:@"question"])
        {
            roomMessage.type = isMessageOutGoing == TRUE?   @(MessageTypeOutgoingAnswer) : @(MessageTypeIncomingAnswer);
            roomMessage.questionStr = dictionary[@"question"]; //[[zingelement elementForName:@"question"] stringValue];
            roomMessage.answerStr = dictionary[@"answer"]; //[[zingelement elementForName:@"answer"] stringValue];
        }
        else if ([type isEqualToString:@"normal"])
        {
            roomMessage.type = isMessageOutGoing == TRUE?   @(MessageTypeOutgoingNormal) : @(MessageTypeIncomingNormal);
            roomMessage.body = dictionary[@"messagedata"]; // [[zingelement elementForName:@"messagedata"] stringValue];
        }
        else if ([type isEqualToString:@"presence"])
        {            
            roomMessage.type = @(MessageTypePresence);
            roomMessage.body = dictionary[@"message"];
        }
        
        if (dictionary[@"userimageurl"] != nil)
        {
            roomMessage.userimageurl = dictionary[@"userimageurl"];
        }
    }
     */
...
//    if ([dictionary[@"sender"] isEqualToString:[[[SessionManager sharedInstance] currentSession] jabberId]] && [dictionary[@"type"] isEqualToString:@"presence"])
//    {
//        return;
//    }
...
- (void)handleIncomingMessage:(XMPPMessage *)message room:(XMPPRoom *)room
{
    ...
    //PI: check if the most recent message saved in data base is smaller than the history message we got
    //for avoiding duplicate messages
    /* commented for testing
     
    NSDate *date = [self mostRecentMessageTimestampForRoom:room.roomJID stream:room.xmppStream inContext:nil];
    if (date != nil)
    {
        NSDate *remoteTimestamp = [message delayedDeliveryDate];
        if (remoteTimestamp != nil)
        {
            NSComparisonResult result = [remoteTimestamp compare:date];

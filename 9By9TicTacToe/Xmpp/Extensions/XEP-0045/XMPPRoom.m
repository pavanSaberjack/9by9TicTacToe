...
- (void)createRoomUsingNickName:(NSString *)desiredNickname
{
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        [presence addAttribute:[NSXMLNode attributeWithName:@"from" stringValue:[[xmppStream myJID] bare]]];
		[presence addChild:x];
...
- (void)acceptInstantRoomConfiguration
{
    ...
    // get the instant room
    [self fetchConfigurationForm];
}
...
- (void)dispatchInvitationToUser:(NSString *)user toJoinChatRoom:(NSString *)chatRoom
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		XMPPMessage *message = [XMPPMessage messageWithType:nil to:[XMPPJID jidWithString:user]];
        NSXMLNode * from = [NSXMLNode attributeWithName:@"from" stringValue:[[xmppStream myJID] bare]];
        [message addAttribute:from];
        
        NSXMLElement *x = [NSXMLElement elementWithName:@"x" xmlns:XMPPMUCInviteNamespace];
        
        NSXMLNode * jid = [NSXMLNode attributeWithName:@"jid" stringValue:[NSString stringWithFormat:@"%@@conference.%@",chatRoom,[[xmppStream myJID] domain]]];
        NSXMLNode * reason = [NSXMLNode attributeWithName:@"reason" stringValue:[NSString stringWithFormat:@"Good Discussions are available here"]];
        
        [message addAttribute:[NSXMLNode attributeWithName:@"from" stringValue:[[xmppStream myJID] bare]]];
        [x addAttribute:jid];
        [x addAttribute:reason];
